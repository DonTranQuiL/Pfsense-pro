...
async def test_binary_sensors_active(hass: HomeAssistant, mock_pfsense_client):
    ...
    entry = MockConfigEntry(
        domain=DOMAIN,
        title="router.local",
        data={"url": "https://192.168.1.1", "username": "admin", "password": "password"},
        options={"device_tracker_enabled": False},
        entry_id="pfsense_binary_test_on",
        version=2, # <--- THE FIX: Bypass migration by telling HA this is already v2
    )
    entry.add_to_hass(hass)

    with patch("custom_components.pfsense.pfSenseClient", return_value=mock_pfsense_client), \
    ...
    notices_sensor = hass.states.get("binary_sensor.router_local_pending_notices_present")
    assert notices_sensor is not None
    assert notices_sensor.state == STATE_ON
    
    # Check attributes and device class
    assert notices_sensor.attributes.get("device_class") == BinarySensorDeviceClass.PROBLEM
    assert notices_sensor.attributes.get("pending_notices") == {"id_1": "Update available"}
