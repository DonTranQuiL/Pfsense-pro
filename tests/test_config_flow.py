...
async def test_form_user_success(hass):
    ...
    with patch("custom_components.pfsense.config_flow.Client") as mock_client_cls, \
        ...
        # pfSense has a second step for device selection
        if result2["type"] == FlowResultType.FORM and result2["step_id"] == "device_tracker":
            result2 = await hass.config_entries.flow.async_configure(
                result2["flow_id"], user_input={CONF_DEVICES: []}
            )
...
async def test_options_flow(hass):
    ...
    with patch("custom_components.pfsense.config_flow.Client") as mock_client_cls:
        ...
        # FIX: The initial options step is named init
