...
async def test_config_flow_success(hass: HomeAssistant):
    ...
    with patch("custom_components.pfsense.config_flow.Client", return_value=mock_client), \
...
async def test_config_flow_invalid_auth(hass: HomeAssistant):
    ...
    mock_client.get_system_info.side_effect = xmlrpc.client.Fault(1, "Invalid username or password")
...
async def test_coordinator_sensor_extraction(hass: HomeAssistant):
    ...
    mock_client.get_telemetry.return_value = {
        ...
        "cpu": {
            "used_percent": 15,
            "frequency": {"current": 2000, "max": 3000}
        },
        "system": {
            "temp": 45.5,
            "boottime": 1600000000,
    ...
    with patch("custom_components.pfsense.pfSenseClient", return_value=mock_client), \
